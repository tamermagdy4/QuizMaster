...
export function About() {
  ...
  const contacts = [
    { icon: 'whatsapp', name: 'WhatsApp', description: english ? 'Contact me directly' : 'تواصل معي مباشرة', href: whatsappUrl, external: true },
  ...
  return (
    <main dir={english ? 'ltr' : 'rtl'} className="mx-auto w-full max-w-5xl space-y-6">
      ...
      <section><div className="mb-4"><p className="eyebrow">{english ? 'Contact' : 'التواصل'}</p><h2 className="mt-1 text-2xl font-black text-ink">{english ? 'Contact me' : 'تواصل معي'}</h2></div><div className="grid gap-4 sm:grid-cols-2">{contacts.map((contact) => <a key={contact.name} href={contact.href} target={contact.external ? '_blank' : undefined} rel={contact.external ? 'noopener noreferrer' : undefined} aria-label={`${contact.name}: ${contact.description}`} className="group flex items-center gap-4 rounded-2xl border border-border-soft bg-white p-4 shadow-panel transition hover:-translate-y-0.5 hover:border-border-strong hover:shadow-raised"><span className="flex h-11 w-11 items-center justify-center rounded-xl bg-teal/10 text-teal transition group-hover:bg-teal/20">{contact.icon === 'whatsapp' ? <MessageCircle className="h-5 w-5" /> : <Mail className="h-5 w-5" />}</span><span><strong className="block text-sm font-black text-ink">{contact.name}</strong><span className="mt-1 block text-xs text-muted">{contact.description}</span></span><span className="ms-auto text-muted transition group-hover:text-teal" aria-hidden>↗</span></a>)}</div></section>
      <GlassCard strong className="border-gold/40 bg-gold/5"><h2 className="text-xl font-black text-ink">{english ? 'Support the project' : 'دعم المشروع'}</h2><p className="mt-2 text-sm leading-7 text-ink-2">{english ? `If you enjoy ${brand} and would like to support the project or share your feedback, you can contact me through any of the channels above.` : `إذا أعجبك ${brand} وترغب في دعم المشروع أو مشاركة رأيك، يمكنك التواصل معي عبر أي من وسائل التواصل الموجودة بالأعلى.`}</p></GlassCard>
    </main>
  )
}
